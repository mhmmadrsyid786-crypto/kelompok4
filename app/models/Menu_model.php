...
class Menu_model {
    ...
    public function tambahDataMenu($data) {
        $query = "INSERT INTO " . $this->table . " (nama_menu, jenjang, kategori, kalori, protein, status) VALUES (:nama_menu, :jenjang, :kategori, :kalori, :protein, :status)";
        $this->db->query($query);
    ...
    public function ubahDataMenu($data) {
        $query = "UPDATE " . $this->table . " SET nama_menu = :nama_menu, jenjang = :jenjang, kategori = :kategori, kalori = :kalori, protein = :protein, status = :status WHERE id_menu = :id_menu";
        $this->db->query($query);
        $this->db->bind('nama_menu', $data['nama_menu']);
        $this->db->bind('jenjang', $data['jenjang']);
        $this->db->bind('kategori', $data['kategori']);
